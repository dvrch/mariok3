<script lang="ts">
  ...
  useTask((delta) => {
    ...
    const joystickX = gameStore.joystickX;
    const driftButton = gameStore.driftButton; // treated as Jump logic
    const itemButton = gameStore.itemButton;
    const menuButton = gameStore.menuButton;
    ...
    } else if (currentSpeed > maxSpeed && effectiveBoost > 0) {
    ...
    // No Reverse in Touch? Original code didn't have explicit reverse button logic in the frame loop block visible.
    // It relied on auto-acccel.
    ...
    // Drifting
    if (jumpIsHeld && currentSteeringSpeed > 0 && joystickX < -0.1 && !driftRight) {
      driftLeft = true;
    }
    if (jumpIsHeld && currentSteeringSpeed > 0 && joystickX > 0.1 && !driftLeft) {
      driftRight = true;
    }
